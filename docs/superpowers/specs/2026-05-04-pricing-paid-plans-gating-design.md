...
└─ getPricingVisibility() → { showPaidPlans: boolean, reason: 'guest' | 'under' | 'reached' | 'paid' }
   ...
   ├─ 已登入 + 付費 (PREMIUM/ENTERPRISE/PUBLISHER) → { showPaidPlans: true, reason: 'paid' }
...
| VIP 帳號 (`prpispace@gmail.com`) | 依實際訂閱狀態決定 | `VIP_EMAILS` 只用於 publisher 審核，不影響 plan。VIP 若有付費訂閱走 `paid`，否則照 quiz count 規則 |
...
| `src/libs/PricingVisibility.test.ts` | **新增** vitest | 4 分支純函數單測（對齊 `fork.test.ts` 風格） |
...
`src/libs/PricingVisibility.test.ts` — 4 分支純函數單測（拆出 `evaluateVisibility(input)` 純邏輯，對齊 `fork.test.ts` 模式，不接 DB）：
- `guest`：`isAuthed=false` → `{ showPaidPlans: false, reason: 'guest' }`
- `paid`：`isAuthed=true, planId=PREMIUM` → `{ true, 'paid' }`
- `reached`：`isAuthed=true, planId=FREE, quizCount=10` → `{ true, 'reached' }`
- `under`：`isAuthed=true, planId=FREE, quizCount=9` → `{ false, 'under' }`
