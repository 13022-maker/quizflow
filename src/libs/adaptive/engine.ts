...
export class AdaptiveEngine {
  ...
  private topologicalSort(graph: KnowledgeGraph): KnowledgeNode[] {
    ...
    const visit = (node: KnowledgeNode): void => {
      ...
      for (const prereqId of node.prerequisites) {
        ...
        if (!prereq) throw new Error(`找不到前置知識點：${prereqId}`);
        visit(prereq);
      }
  ...
  async registerStudent(studentId: string): Promise<StudentState> {
    ...
    if (existing) return existing; // 已有紀錄：直接續用（跨執行恢復進度）
  ...
  async getNextItem(studentId: string): Promise<DispatchResult | null> {
    ...
    if (!targetNode) return null; // 全部精熟，課程完成
  ...
  async updateStudentState(
    ...
    if (!item) throw new Error(`題目不存在：${itemId}`);
    ...
    const timeFactor =
      responseTimeSec <= ENGINE_CONFIG.FAST_ANSWER_SEC ? 1.1
      : responseTimeSec <= ENGINE_CONFIG.SLOW_ANSWER_SEC ? 1.0
      : 0.8;
    const transit = clamp(
