...
export interface AnnotationExchange {
...
export class BloomTutorLayer {
  ...
  async getNextStep(
    ...
    if (pending) return { type: "lesson", lesson: pending.lesson };
  ...
  async askAnnotation(
    ...
    const pending = this.pendingLessons.get(studentId);
    if (!pending) throw new Error(`學生 ${studentId} 沒有進行中的課文，無法劃線提問`);
  ...
  async completeLesson(studentId: string): Promise<void> {
    const pending = this.pendingLessons.get(studentId);
    if (!pending) throw new Error(`學生 ${studentId} 沒有進行中的課文`);
